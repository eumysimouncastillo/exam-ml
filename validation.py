...
def validate_logs(df, exam_start, exam_end):
    '''
    Checks that logs are logically valid, not just formatted correctly.
    exam_start and exam_end must be timezone-aware datetime objects.
    '''
    print(f'[Validation] Starting with {len(df)} rows...')

    # Remove events outside the exam time window
    before = len(df)
    df = df[(df['timestamp'] >= exam_start) & (df['timestamp'] <= exam_end)]
    print(f'[Validation] Removed {before - len(df)} out-of-window events')

    # Remove rows with impossible time jumps (negative time between events)
    ...
    # Remove rows with invalid email, student ID, or action type
    before = len(df)
    df = df[df.apply(_is_row_valid, axis=1)]
    print(f'[Validation] Removed {before - len(df)} rows with invalid field values')

    df = df.reset_index(drop=True)
    print(f'[Validation] Finished with {len(df)} valid rows')
    return df


def _is_row_valid(row):
    '''Returns True if this row passes all field checks. Called once per row.'''
    try:
